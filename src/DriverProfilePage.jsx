...
export default function DriverProfilePage({ seasons, activeSeason }) {
  ...
  // Initialize state with first season if available
  ...
  // Find driver in selected season
                ...
                <tbody>
                  {raceBreakdown.map((race) => (
                    <tr key={race.raceName}>
                      ...
                      <td style={{ ...tdStyle, color: (race.penaltyPoints || 0) > 0 ? "#f87171" : "inherit" }}>
                        {(race.penaltyPoints || 0) > 0 ? `-${race.penaltyPoints}` : "—"}
                      </td>
