...
function getTeamFullName(team) { const names = { JAM: "JA Motorsports", MER: "ME Racing", MMS: "Mayhem Motorsports", NLM: "Nine Line Motorsports", BOM: "Blue Oval Motorsports", Independent: "Independent" }; return names[team] || team || "Unknown Team"; }

export default function TeamDetailPage({ drivers = [], teamDrivers = [], teamStandings = [], standings = [], selectedStanding = null, team = null, initialTeam = "", selectedTeam = "", seasonName = "" }) {
  const teamKey = normalize(selectedTeam || initialTeam || selectedStanding?.team || team?.team);
  const standingsSource = teamStandings.length ? teamStandings : standings;
  const standing = selectedStanding || team || standingsSource.find((t) => normalize(t.team) === teamKey) || null;
  const roster = (teamDrivers.length ? teamDrivers : drivers.filter((d) => normalize(d.team) === teamKey))
    .filter((d) => !normalize(d.name).startsWith("inactive-"))
    .sort((a, b) => (b.points || 0) - (a.points || 0) || Number(a.number || 0) - Number(b.number || 0));
  const fallbackStanding = { team: selectedTeam || initialTeam || "Unknown Team", points: roster.reduce((sum, d) => sum + (d.points || 0), 0), wins: roster.reduce((sum, d) => sum + (d.wins || 0), 0), top3: roster.reduce((sum, d) => sum + (d.top3 || 0), 0), top5: roster.reduce((sum, d) => sum + (d.top5 || 0), 0), drivers: roster.length };
  ...
  return (
    <div style={pageStyle}><div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ ...cardStyle, borderColor: "#d4af37" }}><div style={{ fontSize: 13, opacity: 0.72, marginBottom: 6 }}>{seasonName}</div><div style={{ fontSize: 38, fontWeight: 900 }}>{getTeamFullName(data.team)}</div><div style={{ fontSize: 15, opacity: 0.72, marginTop: 6 }}>Team Detail Page</div></div>
