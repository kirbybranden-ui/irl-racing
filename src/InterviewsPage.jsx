...
export default function InterviewsPage() {
  ...
  function isRemovedDriver(interview) {
    const name = String(interview?.driver_name || "").trim().toLowerCase();
    const number = String(interview?.driver_number || "").trim();

    return name === "vtfan_25" || number === "16";
  }
  ...
  const answeredInterviews = useMemo(() => {
    return interviews
      .filter((interview) => !isRemovedDriver(interview))
      .map((interview) => ({
        ...interview,
  ...
  const filteredInterviews = useMemo(() => {
    return answeredInterviews.filter((interview) => {
      ...
      const matchesSearch =
        driverText.includes(searchText) || qaText.includes(searchText);
